...
void PackMatrixA(int k,float* a,int lda,float* a_to){
	...
	float* a_0p = a,*a_1p = a + lda,*a_2p = a + (lda << 1),*a_3p = a + 3 * lda;
	for (i = 0;i < k;i++){
		a_to[index_to] = *a_0p++;
		a_to[index_to + 1] = *a_1p++;
		a_to[index_to + 2] = *a_2p++;
		a_to[index_to + 3] = *a_3p++;
		index_to += 4;
	}
...
void InnerKernel(int m,int n,int k,float* A,int lda,float* B,int ldb,float* C,int ldc){
	...
	for (j = 0;j < n;j+=4){
		...
		for (i = 0;i < m;i+=4){
			if (j == 0){
				PackMatrixA(k,pa + lda * i,lda,packageA + i * k);
			}

			AddDot4x4(k,packageA + i * k,lda,packageB,4,C + i*ldc+j,ldc);
		}
		pb += 4;
...
void AddDot4x4(int k,float* A,int lda,float* B,int ldb,float* C,int ldc){
	...
	for (p = 0;p < k;p++){
		...
		a_1p = _mm_load1_ps(pa + 1);
		a_2p = _mm_load1_ps(pa + 2);
		a_3p = _mm_load1_ps(pa + 3);

		b_p = _mm_load_ps(pb);

		//printf("a : %5d %5d %5d %5d\n",(int)*pa,(int)*(pa+1),(int)*(pa+2),(int)*(pa+3));
