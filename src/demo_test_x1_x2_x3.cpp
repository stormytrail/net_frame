...
int main(){
	...
	l1.param_num_ = 1;
	l1.param_shape_ = vector<vector<int>>{vector<int>{3,2}};
	...
	vector<vector<int>> max_input_dim(1,vector<int>{3});
	net.ExecuteSequence(max_input_dim);

	//generate train data
	int num_train = 100;
	float* train_data = (float*)malloc(sizeof(float) * 3 * num_train);
	float* train_label = (float*)malloc(sizeof(float) * 2 * num_train);

	int data_range = 10;

	for (int i = 0;i < num_train;i++){
		train_data[3 * i] = rand() % (2 * data_range) - data_range;
		train_data[3 * i + 1] = rand() % (2 * data_range) - data_range;
		train_data[3 * i + 2] = rand() % (2 * data_range) - data_range;
		bool flag = (train_data[3 * i] + train_data[3 * i + 1] + train_data[3*i+2]) > 0 ? true : false;
		if (flag){
			train_label[2 * i] = 1;
			train_label[2 * i + 1] = 0;
		}
...
//			cout << cur_batch_size << endl;
			...
			Atom data("data",vector<int>{cur_batch_size,3});
			Atom label("label",vector<int>{cur_batch_size,2});

			memcpy(data.data,train_data_shifter,sizeof(float) * cur_batch_size*3);
			memcpy(label.data,train_label_shifter,sizeof(float) * cur_batch_size*2);
			train_data_shifter += cur_batch_size * 3;
